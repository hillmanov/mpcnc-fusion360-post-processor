# !!!Modified version of https://github.com/guffy1234/mpcnc_posts_processor!!!

Main changes:

- Turned off z probe options by default (I don't have a z probe)
- Default to not use arcs (not supported by Marlin 2.0)
- Commented out code that adds z probe prompt (No matter what settings you set, it would always try to add z probe gcode)

TODO: 
Add `G92 X0 Y0 Z0` command to the beginning to make the start position the home position. 

The issue I have with my machine is that I can get everything position correctly, and then when I turn on my spindle, the vibrations make the z axis sag about 1mm. If I can position the z axis with the controller first, it will hold, but then the home position is off. I need the home position to be set to where I manually moved it without having to cycle the power. 
